...
class ImageLib {
    ...
    * @ShortDescription      This function is responsible to resize image, 
    */
    public function resizeImageBeforeUpload($thumb_image, $destination, $filename, $height, $width) {
        $image_resize = Image::make($thumb_image->getRealPath());              
        $image_resize->resize($height, $width);

        $destinationPath = storage_path($destination);

        if(!File::exists(storage_path($destination))) {
            // path does not exist
            File::makeDirectory(storage_path($destination), 0755, true, true);
        }
        // // Save the thumbnail to given path
        $upload = $image_resize->save($destinationPath.$filename);
